...
class PoGPE:
    ...
    def fit(self, X, y):
        ...
        for gp_expert, (start, end) in zip(gp_experts, idxs):
            gp_expert.fit_class(X_s[start:end], y_s[start:end])
        self.gp_experts = gp_experts

    def gp_means_vars(self, x):
        for gp_expert in self.gp_experts:
            gp_expert.predict_class(x)

        # Means, variances for each binary class case for each GP regressor (classifier)
        y_preds = np.array([gp_expert.predict_class(x, keep_probs=True, parallel=True, PoE=True) for gp_expert in self.gp_experts])
    ...
    def predict(self, x, keep_probs=False):
        means_gp, vars_gp = gp_means_vars(self, x):
